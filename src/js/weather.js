/*---------------------刷新时自动获取当前IP天气--------------------------*/
	$.get('https://weixin.jirengu.com/weather/ip').done(function(rel){
		$.get('https://weixin.jirengu.com/weather/cityid?location='+rel.data).done(function(rel2){
			setCity(rel2.results[0].name,0)
			$.get('https://weixin.jirengu.com/weather/now?cityid='+rel2.results[0].id).done(function(rel3){
				setToday (rel3.weather[0],0)
			})
			$.get('https://weixin.jirengu.com/weather/future24h?cityid='+rel2.results[0].id).done(function(rel4){
				set24h(rel4,0)
			})
		})
	})
...
/*-------------------------添加城市---------------------------*/
		...
		$('#addWea>input').keyup(function () {
			...
			timer1 = setTimeout(function(){
				$('#addWea>span').css('display','block')
				$.get('https://weixin.jirengu.com/weather/cityid?location=' + $that.val()).done(function(rel5){
					$('#addWea>span').css('display','none')
		...
		$('#addWea>ul').on('click','li',function(){
			...
			$.get('https://weixin.jirengu.com/weather/now?cityid='+$(this).attr('id')).done(function(rel3){
				var cloneInner = $('.inner')[0].cloneNode(true)
				$('.content').append($(cloneInner))
				
				setToday (rel3.weather[0],$length)
			})
			$.get('https://weixin.jirengu.com/weather/future24h?cityid='+ $(this).attr('id')).done(function(rel4){
				set24h(rel4,$length)
